...
BEGIN
  ...
  -- Seed unit Sport Center (lookup dinamis) jika belum ada
  IF sc_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM mall_units WHERE site_id = sc_id LIMIT 1) THEN
    INSERT INTO mall_units (unit_code, floor, zone, size_m2, status, position_x, position_y, width, height, site_id) VALUES
...
BEGIN
  SELECT id INTO sc_id FROM mall_sites WHERE code = 'SPORT_CENTER_BANDARA';
  IF sc_id IS NULL THEN RETURN; END IF;
