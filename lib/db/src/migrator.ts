...
-- Restore unit menggunakan site id canonical dari code, tanpa PL/pgSQL dollar quoting.
INSERT INTO mall_units (unit_code, floor, zone, size_m2, status, position_x, position_y, width, height, site_id)
SELECT v.unit_code, v.floor, v.zone, v.size_m2, v.status, v.position_x, v.position_y, v.width, v.height, s.id
FROM mall_sites s
CROSS JOIN (VALUES
  ('T-001','1','Zona A',20,'occupied',0,0,2,2),
  ('T-002','1','Zona A',16,'occupied',2,0,2,2),
  ('T-003','1','Zona A',14,'occupied',4,0,2,2),
  ('T-004','1','Zona A',18,'occupied',6,0,2,2),
  ('T-005','1','Zona B',20,'occupied',0,2,2,2),
  ('T-006','1','Zona B',16,'occupied',2,2,2,2),
  ('T-007','1','Zona B',14,'occupied',4,2,2,2),
  ('T-008','1','Zona B',18,'available',6,2,2,2),
  ('T-009','1','Zona A',20,'occupied',0,4,2,2),
  ('T-010','1','Zona A',16,'occupied',4,4,2,2),
  ('T-011','1','Zona B',20,'occupied',6,4,2,2),
  ('T-012','1','Zona A',18,'occupied',0,6,2,2),
  ('T-013','1','Zona B',24,'available',2,6,2,2),
  ('T-014','1','Zona C',30,'available',4,6,2,2),
  ('T-015','1','Zona C',20,'available',6,6,2,2),
  ('T-016','1','Zona C',20,'available',8,6,2,2)
) AS v(unit_code,floor,zone,size_m2,status,position_x,position_y,width,height)
WHERE s.code = 'TOD_M1_BANDARA'
...
FROM mall_sites s
CROSS JOIN (VALUES
  ('SC-01','1','Lapangan',200,'occupied',0,0,4,4),
  ('SC-02','1','Lapangan',200,'occupied',4,0,4,4),
  ('SC-03','1','Lapangan',200,'occupied',8,0,4,4),
  ('SC-04','1','Fasilitas',120,'occupied',0,4,4,4),
  ('SC-05','1','Fasilitas',80,'occupied',4,4,2,2),
  ('SC-06','1','Fasilitas',80,'occupied',6,4,2,2),
  ('SC-07','1','Tenant',40,'occupied',8,4,2,2),
  ('SC-08','1','Tenant',40,'occupied',8,6,2,2)
) AS v(unit_code,floor,zone,size_m2,status,position_x,position_y,width,height)
WHERE s.code = 'SPORT_CENTER_BANDARA'
ON CONFLICT (site_id, unit_code) DO NOTHING;
    `.trim(),
  },
  {
...
DO $sport_center$ DECLARE
  ...
  t1 int; t2 int; t3 int; t4 int; t5 int; t6 int; t7 int; t8 int;
BEGIN
  SELECT id INTO sc_id
  FROM mall_sites
  WHERE code = 'SPORT_CENTER_BANDARA';

  IF sc_id IS NULL THEN
    RAISE EXCEPTION 'Canonical site SPORT_CENTER_BANDARA not found';
  END IF;
  ...
  ELSE
    ...
    UPDATE mall_units SET status = 'occupied' WHERE site_id = sc_id AND unit_code LIKE 'SC-%';
  END IF;
...
END $sport_center$;
  ...
  },
  {
    ...
    sql: `
DO $sport_center_tenants$ DECLARE
  sc_id int;
  t1 int; t2 int; t3 int; t4 int; t5 int; t6 int; t7 int; t8 int;
BEGIN
  SELECT id INTO sc_id
  ...
  IF NOT EXISTS (SELECT 1 FROM tenants WHERE site_id = sc_id LIMIT 1) THEN
    ...
    INSERT INTO tenant_bookings
      (tenant_id, unit_code, floor, start_date, end_date, rent_amount, deposit_amount,
       ...
       period_label, booking_type, billing_cycle, order_number, contract_number)
    VALUES
      (t1, 'SC-01', '1', '2024-01-01', '2026-12-31', 8000000,  16000000, 'aktif', 'active', 'active', 'lunas', sc_id, 'Jan 2024 - Des 2026', 'sewa', 'bulanan', 'SC-ORD-2024-0001', 'SC-KTR-2024-0001'),
      (t2, 'SC-02', '1', '2024-03-01', '2027-02-28', 8000000,  16000000, 'aktif', 'active', 'active', 'lunas', sc_id, 'Mar 2024 - Feb 2027', 'sewa', 'bulanan', 'SC-ORD-2024-0002', 'SC-KTR-2024-0002'),
      (t3, 'SC-03', '1', '2023-07-01', '2025-06-30', 8000000,  16000000, 'aktif', 'active', 'active', 'lunas', sc_id, 'Jul 2023 - Jun 2025', 'sewa', 'bulanan', 'SC-ORD-2023-0001', 'SC-KTR-2023-0001'),
      (t4, 'SC-04', '1', '2024-06-01', '2027-05-31', 6000000,  12000000, 'aktif', 'active', 'active', 'lunas', sc_id, 'Jun 2024 - Mei 2027', 'sewa', 'bulanan', 'SC-ORD-2024-0003', 'SC-KTR-2024-0003'),
      (t5, 'SC-05', '1', '2025-01-01', '2027-12-31', 4500000,   9000000, 'aktif', 'active', 'active', 'lunas', sc_id, 'Jan 2025 - Des 2027', 'sewa', 'bulanan', 'SC-ORD-2025-0001', 'SC-KTR-2025-0001'),
      (t6, 'SC-06', '1', '2024-09-01', '2026-08-31', 4500000,   9000000, 'aktif', 'active', 'active', 'lunas', sc_id, 'Sep 2024 - Agu 2026', 'sewa', 'bulanan', 'SC-ORD-2024-0004', 'SC-KTR-2024-0004'),
      (t7, 'SC-07', '1', '2025-03-01', '2027-02-28', 2500000,   5000000, 'aktif', 'active', 'active', 'lunas', sc_id, 'Mar 2025 - Feb 2027', 'sewa', 'bulanan', 'SC-ORD-2025-0002', 'SC-KTR-2025-0002'),
      (t8, 'SC-08', '1', '2025-05-01', '2027-04-30', 2500000,   5000000, 'aktif', 'active', 'active', 'lunas', sc_id, 'Mei 2025 - Apr 2027', 'sewa', 'bulanan', 'SC-ORD-2025-0003', 'SC-KTR-2025-0003');

    -- Update semua unit Sport Center jadi occupied
    UPDATE mall_units SET status = 'occupied' WHERE site_id = sc_id;

  END IF;
END $sport_center_tenants$;
    `.trim(),
  },
  {
...
MIGRATIONS.push({
  name: "0072_consolidated_invoices",
  sql: `
